...
const resetPwRules = computed(() => {
  const pw = String(resetPasswordForm.value.newPassword || '')
  return [
    { label: '8자 이상', pass: pw.length >= 8 },
    { label: '영문 포함', pass: /[a-zA-Z]/.test(pw) },
    { label: '숫자 포함', pass: /[0-9]/.test(pw) },
    { label: '특수문자 포함', pass: /[!@#$%^&*()_+\-=\[\]{}|;:'",.<>?/`~]/.test(pw) }
  ]
})
const allResetPwRulesPass = computed(() => resetPwRules.value.every((rule) => rule.pass))
const canSubmitResetPassword = computed(() => {
  const newPw = String(resetPasswordForm.value.newPassword || '')
  const confirmPw = String(resetPasswordForm.value.confirmPassword || '')
  return allResetPwRulesPass.value && newPw.length > 0 && confirmPw.length > 0 && newPw === confirmPw
})
