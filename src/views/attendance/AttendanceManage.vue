...
const saveChanges = async () => {
  ...
  if (!selectedItem.value?.employeeId) {
    alert('관리자용 근태 목록 조회 API가 없어 수정 대상을 식별할 수 없습니다.')
    return
  }

  await store.updateDailyAttendance({
    targetEmployeeId: selectedItem.value.employeeId,
    workDate: selectedItem.value.date,
    newCheckInTime: editForm.value.checkIn || null,
    newCheckOutTime: editForm.value.checkOut || null,
    newStatus: statusToBackend(editForm.value.status),
