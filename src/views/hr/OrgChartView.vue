<template>
  <div class="org-page">
    <div class="breadcrumb">인사 / 조직도</div>
    ...
    <section class="split-layout">
          ...
          <template v-else>
            ...
            >
              <template v-if="row.kind === 'node'">
                <span class="chevron" :class="{ open: isOpen(row.node.id) }">
                  {{ row.expandable ? '▸' : '•' }}
                </span>
      ...
      <article class="panel right-panel">
        ...
        <div class="profile-scroll">
          <div v-if="activeMembers.length === 0" class="empty">
            선택된 조직의 구성원이 없습니다.
          </div>
...
.tree-scroll { flex: 1; overflow-y: auto; padding: 8px 8px 12px; }
.profile-scroll { flex: 1; overflow-y: auto; padding: 12px; }
...
.profile-avatar span { color: #fff; font-size: .86rem; font-weight: 800; }

.profile-head { display: flex; align-items: center; gap: 6px; }
.profile-head strong { color: var(--gray800); font-size: .98rem; }
